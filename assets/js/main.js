...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll(".favorite__button").forEach(button => {
    button.addEventListener("click", function () {
      ...
      const srcCompleto = card.querySelector(".favorite__img")?.getAttribute("src") || "";
let imagen = srcCompleto;

// Si contiene una ruta tipo 'assets/img/...' => extrae solo el nombre
if (srcCompleto.includes('/')) {
  imagen = srcCompleto.split('/').pop(); // Ej: dona.png o default.jpg
}
console.log("Imagen guardada en carrito:", imagen);
...
console.log({
  ...
  .then(data => {
      // esto ayuda a ver si hay errores PHP
...
function actualizarCarritoUI() {
 ...
 carrito.forEach((item, index) => {
  console.log("Imagen renderizada:", `/assets/img/${item.imagen}`);
  const itemHTML = `
    <div class="item-carrito" style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;">
      ...
      <button class="btn-eliminar" data-index="${index}" style="margin-left: 10px; background: red; color: white; border: none; border-radius: 4px; cursor: pointer; padding:8px;">X</button>
    </div>
  `;
